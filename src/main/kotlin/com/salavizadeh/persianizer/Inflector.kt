...
import com.salavizadeh.persianizer.exceptions.NoRuleFoundException
...
fun String.titleize(): String {
  ...
  this.replace("-", " ").replace("_", " ").capitalize()
      .forEach {
        if (Character.isWhitespace(it) || !Character.isLetterOrDigit(it)) previousWasWhitespace =
            true
...
private fun String.pluralizer(): String {
  ...
  if (endswith != null)
    found = this.replace(endswith.component1(), endswith.component2())

  val excep = exceptions().firstOrNull { this == it.component1() }

  if (excep != null)
    found = excep.component2()
...
private fun String.singularizer(): String {
  if (unCountable().contains(this)) return this
  val excep = exceptions().firstOrNull { this == it.component2() }
