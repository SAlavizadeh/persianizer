...
fun String.toQuantity(
  ...
  if (showQuantityAs == ShowQuantityAs.Numeric) {
    ...
    if (quantity == 1L || quantity == -1L)
      return "$formattedQuantity ${this.singularize(Plurality.CouldBeEither)}"
  ...
  if (showQuantityAs == ShowQuantityAs.None) {
    ...
    if (quantity == 1L || quantity == -1L)
      return this.singularize(Plurality.CouldBeEither)
  ...
  if (showQuantityAs == ShowQuantityAs.Words) {
    if (quantity == 1L || quantity == -1L)
      return "${quantity.toWords()} ${this.singularize(Plurality.CouldBeEither)}"
